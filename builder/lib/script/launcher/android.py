...
from botter.bot import send_file
...
class AndroidLauncher(Launcher):
    ...
    def generate(self):
        ...
        if self.need_build:
            self._build_app()
            # send_file(self.get_apk_path(self.application.bundle),
            #          f"[Android] Name: {self.application.app_name} | Version:{self.version_name}")
            try:
                self.send_apk(self.email, self.get_apk_path(self.application.bundle, False), "Client", self.version_name)
            except Exception:
                self.send_apk(self.email, self.get_apk_path(self.application.bundle, True), "Client", self.version_name)
    ...
    def _prepare_res(self, res_helper):
        ...
        res_helper.replace_facebook_id(self.application.facebook_app_id)
    ...
    @staticmethod
    def get_apk_path(bundle, is_new):
        type = "/release" if is_new else ""
        path = merge(config.FINISH_PATH, "gootax_app_android_" + bundle.replace(".", "_"), "app/build/outputs/apk" + type + "/app-release.apk")
