...
class AndroidDriverNewConfigurator:
    ...
    def configure_by_app(self, config: DriverApplicationNew):
        ...
        self.put_string(ALLOW_ROOT, config.push_key)
        self.put_string(SENDER_ID, config.push_key)
        self.put_obj(TYPE_APP, config.app_type)
        self.put_obj(APP_VERSION, config.version_app)
        self.put_string(YANDEX_KEY, config.yandex_key)
        self.put_bool(ALLOW_ROOT, config.allow_root)
