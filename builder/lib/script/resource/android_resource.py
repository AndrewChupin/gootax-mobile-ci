...
from std.config import COUNTRY_LIST, COUNTRY_DRIVER_LIST, COUNTRY_DRIVER_NEW_LIST
...
THEME_FIELDS = ["splash_bg", "accent_bg", "accent_text", "menu_bg", "menu_text", "menu_stroke", "content_bg",
                ...
                'color_text_button_inner', 'color_background_button_inner']
...
class AndroidResourceHelper:
    ...
    def replace_google_driver_service(self, is_new: bool):
        res_type = 'google_driver_new' if is_new else "google_driver"
    ...
    def replace_mipmap(self, name, is_new: bool):
        res_type = 'android_driver_new' if is_new else 'android_driver'
        mipmap = "/mipmap"
        logo = f"/{name}"
        png = ".png"
    ...
    def replace_drawable(self, name, is_new: bool):
        res_type = 'android_driver_new' if is_new else 'android_driver'
        drawable = "/drawable"
        splash = f"/{name}"
        png = ".png"
    ...
    def replace_app_name(self, new_name, type="client", is_new: bool = False):
        string_tag = "string"
        name_tag = "name"
        ...
        if type == "client":
            ...
            counties = COUNTRY_DRIVER_NEW_LIST if is_new else COUNTRY_DRIVER_LIST
    ...
    def replace_facebook_id(self, facebook_id):
        values = "/values"
        app_ids = "/appIds.xml"
        ...
        file_path = merge(self.finish_path, RES_PATH, values, app_ids)
        ...
        for line in root.findall(string_tag):
            if line.get(name_tag) == "facebook_app_id":
                line.text = facebook_id
            elif line.get(name_tag) == "fb_login_protocol_scheme":
                line.text = f"fb{facebook_id}"
        tree.write(file_path)
