...
from backend.model.repository.driver_app_new_repository import DriverApplicationNewRepository
from backend.model.helper import driver_app_new_helper
...
api_driver_app_new_list = "/driver_app_new_list"
api_driver_app_new_save = "/driver_app_new_save"
api_driver_app_new_info = "/driver_app_new_info/<app_id>"
api_driver_app_new_create = "/driver_app_new_create"
api_driver_app_new_update = "/driver_app_new_update"
api_driver_app_new_build = "/driver_app_new_build"
api_driver_app_new_delete = "/driver_app_new_delete"

driver_new_controller = Blueprint(
    name="driver_new",
    import_name=__name__
)


@driver_new_controller.route(api_driver_app_new_list, methods=[api.get])
def driver_app_list():
    ...
    # Render View
    return render_template("driver-app-new-list.html",
                           apps=apps)


@driver_new_controller.route(api_driver_app_new_save, methods=[api.post])
def driver_app_save():
    if request.method == api.post:
        try:
            ...
            # Application
            result = driver_app_new_helper.generate_app_json(request.form)  # Form -> JSON
            print(result)
            new_app = driver_app_new_helper.parse_app_by_json(result)  # JSON -> App
    ...
    return BuildAction.create_action_build(action=BaseAction.REDIRECT_ACTION,
                                           ...
                                           data=BuildAction.data_created_build(api_driver_app_new_list))


@driver_new_controller.route(api_driver_app_new_info, methods=[api.get])
def driver_app_info(app_id: int):
    ...
    response: Response = make_response(render_template("driver-app-new-info.html",
                                                       app=application,
                                                       countries=countries,
                                                       android_driver=res_saved["android_driver_new"] or False,
                                                       google_driver=res_saved["google_driver_new"] or False
                                                       ))
...
@driver_new_controller.route(api_driver_app_new_create, methods=[api.get])
def driver_app_create():
    ...
    # Render View
    return render_template("driver-app-new-create.html", countries=countries)


@driver_new_controller.route(api_driver_app_new_build, methods=[api.post])
def driver_app_build():
    if request.method == api.post:
        ...
        try:
            ...
            build_params = build_helper.generate_driver_build(form, config.ROOT_BUNDLE_DRIVER_NEW)
...
@driver_new_controller.route(api_driver_app_new_update, methods=[api.post])
def driver_app_update():
    if request.method == api.post:
        ...
        try:
            ...
            application = driver_app_new_helper.parse_app_by_json(result)  # JSON -> App
        ...
        try:
            ...
            return redirect(f"/driver_app_new_info/{index}")
        except BaseError as e:
            traceback.print_exc()
            error_response = e.generate_error()
            logging.error(error_response)
...
@driver_new_controller.route(api_driver_app_new_delete, methods=[api.post])
def app_delete():
    if request.method == api.post:
        form = request.form
        index = validate_field(form["id"])
