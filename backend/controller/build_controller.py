...
from backend.model.helper import build_helper, build_config_helper
...
from backend.model.repository import theme_rep, build_config_repository
...
def app_build():
    if request.method == api.post:
        ...
        try:
            ...
            build_config = build_config_helper.generate_build_config(form, index)
        ...
        build_config_db = build_config_repository.BuildConfigRepository(config.DATA_URL)

        exist_build_config = build_config_db.get_by_app_id(index)

        if exist_build_config:
            build_config.id = exist_build_config.id
            build_config_db.update(build_config)
        else:
            build_config_db.create(build_config)
...
@build_controller.route(api.post_driver_new_update_build_status, methods=[api.post])
