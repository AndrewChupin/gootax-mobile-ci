...
ANDROID_DRIVER_NEW = "android_driver_new"
GOOGLE_DRIVER_NEW = "google_driver_new"
...
class ResourceManager:
    ...
    def generate_driver_google_res(self, file, name: str, driver_type: str) -> None:

        temp_res = merge(self.temp_path, name)
        path_res_temp = merge(self.temp_path, name, driver_type)
        path_to_type = merge(self.res_path, name, driver_type)
        ...
        if res.allowed_json(file.filename):
            res.save_json(file, path_res_temp)
            if self.validate_res_by_type(driver_type, path_res_temp):
                res.save_temp(path_res_temp, path_to_type)
    ...
    def validate_res_by_type(res_type: str, path_res_type: str) -> bool:
        ...
        elif res_type == ANDROID_DRIVER_NEW:
            is_valid = res.validate_driver_images(path_res_type)
        elif res_type == GOOGLE_DRIVER_NEW:
            is_valid = res.validate_google_push(path_res_type)
        else:
            raise ResourceError(subcode=ResourceError.res_not_found,
                                data=ResourceError.res_not_found_mes % res_type)
    ...
    def generate_res_files(self, files, name: str):
        ...
        if files:
            ...
            if GOOGLE_DRIVER in files and files[GOOGLE_DRIVER].filename != '':
                self.generate_driver_google_res(files[GOOGLE_DRIVER], name, GOOGLE_DRIVER)
            if GOOGLE_DRIVER_NEW in files and files[GOOGLE_DRIVER_NEW].filename != '':
                self.generate_driver_google_res(files[GOOGLE_DRIVER_NEW], name, GOOGLE_DRIVER_NEW)
            ...
            if ANDROID_DRIVER_NEW in files and files[ANDROID_DRIVER_NEW].filename != '':
                self.generate_zip_res(files[ANDROID_DRIVER_NEW], name, ANDROID_DRIVER_NEW)


    def validate_resources(self, name: str, with_remote: bool=False) -> dict:
        RES_TYPES = [GOOGLE, ANDROID, IOS, ANDROID_DRIVER, GOOGLE_DRIVER, ANDROID_DRIVER_NEW, GOOGLE_DRIVER_NEW]
        result = {}
